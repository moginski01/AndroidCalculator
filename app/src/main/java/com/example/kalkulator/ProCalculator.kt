...
class ProCalculator : AppCompatActivity() {
    var result = "0"

    @SuppressLint("MissingInflatedId")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_pro_calculator)
        ...
        button_equal.setOnClickListener {
            ...
            if(where_operation>=result.length-1){
                ...
                Toast.makeText(this, "Błędne dane!", Toast.LENGTH_SHORT).show()
            }else if(where_operation==0 || result.length==1){
...
//dodać toast
                    ...
                    if(second_digit.compareTo(BigDecimal.ZERO)<0){
                        second_digit = second_digit.multiply(BigDecimal(-1))
                        result = Math.pow(first_digit.toDouble(),second_digit.toDouble()).toString()



                        result = BigDecimal.ONE.divide(result.toBigDecimal(),6,RoundingMode.HALF_UP).toString()
                    }else if(second_digit.equals(BigDecimal.ZERO)){
                        result = "1"
                    }else{
                        result = Math.pow(first_digit.toDouble(),second_digit.toDouble()).toString()
                    }
        ...
        button_sqrt.setOnClickListener {
            ...
            for(i in result){
                if(i=='-' && counter==0){
                    ...
                    if_found_any_operation=1
                    break
                }
                counter+=1
            }

            if(if_found_any_operation==1){
                Toast.makeText(this, "Nieodpowiednie dane!", Toast.LENGTH_SHORT).show()
            }else{
                var temp_res = result.toBigDecimal()

                if(temp_res.compareTo(BigDecimal.ZERO)<=0){
                    Toast.makeText(this, "Nieodpowiednie dane!", Toast.LENGTH_SHORT).show()
                }else{
                    result = sqrt(result.toDouble()).toString()
                }
            }
            resultText.setText(result)
        }
        ...
        button_log.setOnClickListener {
            ...
            if(if_found_any_operation==1){
                ...
                if(temp_res.compareTo(BigDecimal.ZERO)<=0){
                    ...
                    result = log10(result.toDouble()).toString()
                }
            }
            resultText.setText(result)
        }
        ...
        button_ln.setOnClickListener {
            ...
            if(if_found_any_operation==1){
                ...
                if(temp_res.compareTo(BigDecimal.ZERO)<=0){
                    ...
                    result = ln(result.toDouble()).toString()
                }
            }
            resultText.setText(result)
        }

        val button_x2 = findViewById<Button>(R.id.button_x2)
        button_x2.setOnClickListener {
            var counter = 0
            var if_found_any_operation = 0
            for(i in result){
                if(i=='-' && counter==0){
            ...
            if(if_found_any_operation==1){
                ...
                var temp_res = result.toBigDecimal()
                result = result.toBigDecimal().pow(2).toString()
            }
            resultText.setText(result)
        }

        val button_sin = findViewById<Button>(R.id.button_sin)
        button_sin.setOnClickListener {
            ...
            if(if_found_any_operation==1){
                ...
                var temp_res = result.toBigDecimal().toDouble()
                result = sin(temp_res).toString()
            }
            resultText.setText(result)
        }

        val button_cos = findViewById<Button>(R.id.button_cos)
        button_cos.setOnClickListener {
            ...
            if(if_found_any_operation==1){
                ...
                var temp_res = result.toBigDecimal().toDouble()
                result = cos(temp_res).toString()
            }
            resultText.setText(result)
        }

        val button_tan = findViewById<Button>(R.id.button_tan)
        button_tan.setOnClickListener {
            ...
            if(if_found_any_operation==1){
                ...
                var temp_res = result.toBigDecimal().toDouble()
                result = tan(temp_res).toString()
            }
            resultText.setText(result)
        }

        val button_percent = findViewById<Button>(R.id.button_percent)
        button_percent.setOnClickListener {
            ...
            if(if_found_any_operation==1){
                ...
                if(result[0]=='-'){
                    result = result.toBigDecimal().divide(BigDecimal("100")).multiply(BigDecimal("-1")).toString()

                }else{
                    result = result.toBigDecimal().divide(BigDecimal("100")).toString()
                }
            }
            resultText.setText(result)
        }

        val button_xy = findViewById<Button>(R.id.button_xy)
